...
export default function HomeScreen() {
  ...
  React.useEffect(() => {
    const loadToken = async () => {
      try {
        ...
        Alert.alert(tr('error'), tr('failedSavedSession'));
      }
    };
    loadToken();
  }, []);
  ...
  const logout = async (message?: string) => {
    ...
    setContactNo('');
    setAddress('');
    if (message) Alert.alert(tr('sessionExpired'), message);
  };

  const checkResponse = async (res: Response) => {
    if (res.status === 401) {
      await logout(tr('sessionExpiredMessage'));
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      setToken(data.token);
      Alert.alert(tr('success'), tr('authSuccess', { action: isRegister ? tr('register') : tr('login') }));
    } catch (error) {
      console.error('Auth error:', error);
      Alert.alert(tr('error'), error instanceof Error ? error.message : tr('error'));
    } finally {
      setLoading(false);
    }
  };
        ...
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          {!token ? (
            <View style={styles.profileSection}>
              <Text style={styles.sectionTitle}>{isRegister ? tr('register') : tr('login')}</Text>
              <TextInput
              ...
              <Button title={isRegister ? tr('register') : tr('login')} onPress={handleAuth} disabled={loading} />
              <TouchableOpacity onPress={() => setIsRegister(!isRegister)} style={{ marginTop: 10 }}>
                <Text style={{ color: '#33691e', textAlign: 'center' }}>
                  {isRegister ? tr('authToggleLogin') : tr('authToggleRegister')}
                </Text>
              </TouchableOpacity>
            </View>
          ) : (
            <>
              ...
              <View style={styles.featuresContainer}>
                <View style={styles.featureRow}>
                  ...
                  >
                    <Image source={require('../assets/Images/others/krishimitra.jpeg')} style={styles.featureImage} />
                    <Text style={styles.buttonText}>🌾 {tr('krishiMitra')}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.featureButton}
                  ...
                  >
                    ...
                    <Text style={styles.buttonText}>🛒 {tr('agroMart')}</Text>
                  </TouchableOpacity>
                </View>
                <View style={styles.featureRow}>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <Text style={styles.buttonText}>🚜 {tr('agroRent')}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.featureButton}
                  ...
                  >
                    ...
                    <Text style={styles.buttonText}>🛍️ {tr('agriKart')}</Text>
                  </TouchableOpacity>
                </View>
                <View style={styles.featureRow}>
